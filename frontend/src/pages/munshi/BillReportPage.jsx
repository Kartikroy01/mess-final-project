...
import { Calendar, Search, FileText, Download, TrendingUp, IndianRupee, Users, ShoppingBag } from "lucide-react";
...
const BillReportPage = ({ hostel }) => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      setError(err.message || "Failed to fetch student bill report");
      setStudents([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDownloadPdf = () => {
    ...
    doc.text(`Hostel: ${hostel || "N/A"}  |  Month: ${month}  |  Diet Rate: Rs ${dietRate || 0}`, 14, 32);
    ...
    doc.save(`Bill_Report_${hostel || "Hostel"}_${month}.pdf`);
  };
      ...
      <Card className="p-4 md:p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end mb-6">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-5 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold shadow-lg shadow-indigo-100 hover:shadow-indigo-200 transition-all active:scale-95 disabled:opacity-55 cursor-pointer border-none outline-none"
          >
        ...
        {loading ? (
                  ...
                  {paginatedStudents.map((s, idx) => {
                    ...
                    return (
                      <tr key={s.studentId} className="hover:bg-indigo-50/20 transition-colors">
                        ...
                        <td className="px-6 py-4 text-center text-slate-505">₹{dietRate || 0}</td>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-100 mt-4 text-sm font-bold text-slate-600">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-4 py-2 border-2 border-slate-150 rounded-xl hover:bg-slate-50 transition cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed select-none bg-white text-slate-705 font-bold"
                  >
                    Previous
                  </button>
          ...
          <div className="text-center py-20 border-2 border-dashed border-slate-100 rounded-3xl bg-slate-50/50">
            ...
            <p className="text-slate-400 text-sm max-w-sm mx-auto">Could not find any students matching your criteria for the month of {month}.</p>
          </div>
        )}
      </Card>
    </div>
