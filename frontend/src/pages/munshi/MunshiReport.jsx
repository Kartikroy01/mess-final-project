...
const ReportsPage = ({ orders = [], onOrderDeleted, currentPage = 1, totalPages = 1, onPageChange }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <Card className="p-4 md:p-8 overflow-visible min-h-[500px]">
        <div className="bg-white/95 backdrop-blur-xl -mx-4 -mt-4 p-4 md:-mx-8 md:-mt-8 md:p-6 mb-0 rounded-t-3xl shadow-sm border-b border-slate-100 flex flex-col lg:flex-row lg:items-center justify-between gap-4 md:gap-6 transition-all duration-300">
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4 w-full lg:w-auto">
             ...
             <div className="relative flex-1 md:w-64">
                ...
                <input 
                  ...
                  className="pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:border-indigo-500 outline-none transition-all w-full"
                />
             </div>
             ...
             <div className="flex items-center gap-2 md:gap-3 w-full md:w-auto">
                <div className="relative group flex-1 md:w-auto">
                   ...
                   <select 
                      ...
                      className="pl-9 pr-6 py-2.5 bg-white border-2 border-slate-100 rounded-xl focus:border-indigo-500 outline-none transition-all font-bold text-slate-700 appearance-none shadow-sm hover:border-slate-200 cursor-pointer text-xs md:text-sm w-full md:min-w-[130px]"
                   >
                ...
                <div className="relative group flex-1 md:w-auto">
                   <select 
                      ...
                      className="pl-4 pr-6 py-2.5 bg-white border-2 border-slate-100 rounded-xl focus:border-indigo-500 outline-none transition-all font-bold text-slate-700 appearance-none shadow-sm hover:border-slate-200 cursor-pointer text-xs md:text-sm w-full md:min-w-[90px]"
                   >
            ...
            <Button onClick={handleDownloadPdf} variant="success" icon={Download} className="w-full md:w-auto px-6 whitespace-nowrap">
              Export PDF
            </Button>
          </div>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-slate-50 shadow-sm transition-[top] duration-300">
              <tr className="border-b border-slate-200">
                <th className="text-left py-4 px-3 md:px-6 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider bg-slate-50">Date</th>
                <th className="text-left py-4 px-3 md:px-6 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider bg-slate-50">Student</th>
                <th className="text-left py-4 px-3 md:px-6 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider bg-slate-50">Room</th>
                <th className="text-left py-4 px-3 md:px-6 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider bg-slate-50">Meal</th>
                <th className="text-center py-4 px-3 md:px-6 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider bg-slate-50">Diet</th>
                <th className="text-left py-4 px-3 md:px-6 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider w-1/4 bg-slate-50">Items</th>
                <th className="text-right py-4 px-3 md:px-6 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider bg-slate-50">Total</th>
                <th className="text-center py-4 px-3 md:px-6 font-bold text-slate-500 text-[10px] md:text-xs uppercase tracking-wider bg-slate-50">Action</th>
              </tr>
            </thead>
                ...
                filteredOrders.map(order => {
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="py-4 px-3 md:px-6 text-[10px] md:text-sm font-medium text-slate-600">
                        <span className="md:hidden">
                          {new Date(order.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }).replace(' ', '')}
                        </span>
                        <span className="hidden md:inline">
                          {new Date(order.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </span>
                        <div className="text-[10px] md:text-xs text-slate-400 font-normal">
                      ...
                      <td className="py-4 px-3 md:px-6">
                          <div className="font-bold text-slate-800 text-xs md:text-sm">
                            <span className="md:hidden">{(order.studentName || '').split(' ')[0]}</span>
                            <span className="hidden md:inline">{order.studentName || 'Unknown'}</span>
                          </div>
                          <div className="text-[10px] md:text-xs text-slate-400 font-medium">#{order.studentRollNo || 'N/A'}</div>
                      </td>
                      <td className="py-4 px-3 md:px-6">
                      ...
                      <td className="py-4 px-3 md:px-6">
                          <span className={`px-2 py-1 rounded-lg text-[10px] md:text-xs font-bold capitalize ${
                        ...
                        <div className="text-sm text-slate-600 font-medium">
                          {(() => {
                            ...
                            if (diet > 0 && !hasExplicitDietItem) {
                                ...
                                itemStrings.unshift(`Std Diet (x${diet})`);
                            }
                      ...
                      <td className="py-4 px-3 md:px-6 text-right">
                          <span className="font-bold text-slate-800 text-xs md:text-sm">₹{order.totalAmount}</span>
                      </td>
                      <td className="py-4 px-3 md:px-6 text-center">
                        <button 
                          onClick={() => handleDelete(order.id || order._id)}
                          className="p-1 md:p-2 text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"
                          title="Delete Order"
                        >
                          <Trash2 size={16} />
                        </button>
